		...
		.controller(
				...
				function($rootScope, $scope, $http, $state, $stateParams,
					...
					$scope.profile = function(id) {
						var params = "iUrl=user/detail.do?id=" + id
								+ "|iLoading=FLOAT";
						httpService
								.callHttpMethod($http, params)
								.success(
...
mainModule.controller('preLoginCtrl', function($rootScope, $scope, $http,
		$state, $stateParams, $timeout, httpService) {
	$scope.getData = function() {
		if ($rootScope.model && $rootScope.model.sessionAdminName) {
			...
			httpService.callHttpMethod($http, params).success(
					function(result) {
						var isSuccess = httpSuccess(result, 'iLoading=FLOAT',
								'0');
						if (!isJson(result)
								|| isSuccess.indexOf('[ERROR]') >= 0) {
							...
							// 已经登陆成功，跳转至后台主页
							if ($rootScope.model
									&& $rootScope.model.sessionAdminName) {
								window.location.href = "admin.do";
							}
							showMessage('warnMessage',
									$rootScope.model.tipMessage, true, 3);
						}
					});
		}
	};
	...
	$scope.login = function(iurl, myLoading) {
		...
		var params = "iUrl=" + iurl + "|iLoading=" + iLoading
				+ "|iPost=POST|iParams=&" + $('#loginForm').serialize();
...
mainModule.controller('preRegisterCtrl', function($rootScope, $scope, $http,
		$state, $stateParams, httpService) {
	$scope.getData = function() {
...
/** ************************菜单列表*************************** */
mainModule.controller('menuCtrl', function($rootScope, $scope, $http, $state,
		$stateParams, httpService) {
	$scope.getData = function(page) {
		if ($("#searchType").val() != null && $("#searchType").val() != '') {
			$stateParams.type = $("#searchType").val();
		}
		var params = "iUrl=menu/list.do|iLoading=FLOAT|iParams=&type="
				+ $stateParams.type;
...
mainModule
		.controller(
				...
				function($rootScope, $scope, $http, $state, $stateParams,
						httpService) {
					$scope.getData = function(page) {
						var params = "iUrl=user/projectUser/list.do|iLoading=FLOAT|iParams=&projectId="
...
mainModule
		.controller(
				...
				function($rootScope, $scope, $http, $state, $stateParams,
						httpService) {
					$scope.getData = function(page) {
						var params = "iUrl=user/comment/list.do|iLoading=FLOAT|iPost=POST|iParams=&articleId="
...
/** ***********************系统属性*********** */
mainModule.controller('propertyCtrl', function($rootScope, $scope, $http,
		$state, $stateParams, httpService) {
	$scope.getData = function(page) {
...
mainModule
		.controller(
				...
				function($rootScope, $scope, $http, $state, $stateParams,
						httpService) {
					$scope.getData = function(page) {
						var params = "iUrl=setting/list.do|iLoading=FLOAT|iPost=true|iParams=&remark="
								+ $("#searchRemark").val()
								+ "&key="
								+ $stateParams.key;
						$rootScope.getBaseData($scope, $http, params, page);
...
mainModule.controller('settingDetailCtrl', function($rootScope, $scope, $http,
		$state, $stateParams, httpService) {
	$scope.getData = function() {
		var params = "iLoading=FLOAT|iUrl=setting/detail.do?id="
				+ $stateParams.id + "&key=" + $stateParams.key + "&type="
				+ $stateParams.type;
		httpService.callHttpMethod($http, params).success(function(result) {
			var isSuccess = httpSuccess(result, 'iLoading=FLOAT');
			if (!isJson(result) || isSuccess.indexOf('[ERROR]') >= 0) {
...
mainModule
		.controller(
				...
				function($rootScope, $scope, $http, $state, $stateParams,
						httpService) {
					$scope.getData = function(page) {
						var params = "iUrl=role/list.do|iLoading=FLOAT|iPost=true|iParams=&roleName="
								+ $("#roleName").val();
						;
						$rootScope.getBaseData($scope, $http, params, page);
					};
...
mainModule
		.controller(
				...
				function($rootScope, $scope, $http, $state, $stateParams,
						httpService) {
					$scope.getData = function(page) {
						var params = "iUrl=user/source/list.do|iLoading=FLOAT|iPost=true|iParams=&name="
								+ $stateParams.name
								+ "&moduleId="
								+ $stateParams.moduleId;
						$rootScope.getBaseData($scope, $http, params, page);
...
mainModule
		.controller(
				'backInterfaceDetailCtrl',
				function($rootScope, $scope, $http, $state, $stateParams,
						httpService) {
					$scope.getRequestExam = function(editerId, targetId, item,
							tableId) {
						var params = "iUrl=user/interface/getRequestExam.do|iLoading=FLOAT|iPost=true|iParams=&"
...
mainModule
		.controller(
				...
				function($rootScope, $scope, $http, $state, $stateParams,
						httpService) {
					$scope.getData = function(page) {
						var params = "iUrl=log/list.do|iLoading=FLOAT|iPost=true|iParams=&modelName="
								+ $stateParams.modelName
								+ "&identy="
								+ $stateParams.identy;
						$rootScope.getBaseData($scope, $http, params, page);
					};
...
mainModule
		.controller(
				...
				function($rootScope, $scope, $http, $state, $stateParams,
						httpService) {
					$scope.getData = function(page) {
						var params = "iUrl=user/project/list.do|iLoading=FLOAT|iPost=true|iParams=&name="
								+ $stateParams.name
								+ "&type="
								+ $stateParams.type
								+ "&myself="
...
mainModule
		.controller(
				...
				function($rootScope, $scope, $http, $state, $stateParams,
						httpService) {
					$scope.getData = function(page) {
						var params = "iUrl=user/module/list.do|iLoading=FLOAT|iParams=&projectId="
								+ $stateParams.projectId;
						$rootScope.getBaseData($scope, $http, params, page);
					};
					$scope.getData();
		...
		.controller(
				...
				function($rootScope, $scope, $http, $state, $stateParams,
					...
					$scope.getData = function(page) {
						...
						params = "iUrl=user/interface/list.do|iLoading=FLOAT|iPost=true|iParams="
								+ params;
						$rootScope.getBaseData($scope, $http, params, page);
					};
					$scope.getData();
...
mainModule.controller('dictionaryInportFromSqlCtrl', function($rootScope,
		$scope, $http, $state, $stateParams, httpService) {
...
mainModule
		.controller(
				...
				function($rootScope, $scope, $http, $state, $stateParams,
					...
					$scope.importCxParams = function(field) {
						try {
							eval("(" + $rootScope.model.param + ")");
						} catch (e) {
							alert("参数必须是正确的JSON格式，json解析出错：" + e);
							return;
						}

						var jsonText = jsonToDiv($rootScope.model.param);
						if (jsonText.length > 0) {
							$rootScope.model[field] = eval("(" + jsonText + ")");
						}
					}
					$scope.importCxResponseParams = function(field) {
						try {
							eval("(" + $rootScope.model.responseParam + ")");
						} catch (e) {
							alert("参数必须是正确的JSON格式，json解析出错：" + e);
							return;
						}

						var jsonText = jsonToDiv($rootScope.model.responseParam);
						if (jsonText.length > 0) {
							$rootScope.model[field] = eval("(" + jsonText + ")");
						}
					}
...
/** ************************后端接口CX列表*************************** */
mainModule.controller('interfaceCxCtrl', function($rootScope, $scope, $http,
		$state, $stateParams, httpService) {
	$scope.getData = function(page) {
		var params = "";
		if ($("#interfaceName").val() != null
...
mainModule.controller('InterfaceEgmasCtrl', function($rootScope, $scope, $http,
	...
	$rootScope.model = {};
	$rootScope.model.customModule = "";
	$rootScope.model.responsiblePerson = "";
	$rootScope.model.interfaceName = "";
	$rootScope.model.url = "";
	$rootScope.model.requestMethod = "";
	$scope.getData = function(page) {
		var params = "";
		params += "&customModule=" + $rootScope.model.customModule;
		params += "&interfaceName=" + $rootScope.model.interfaceName;
		params += "&url=" + $rootScope.model.url;
		params += "&requestMethod=" + $rootScope.model.requestMethod;
		params += "&responsiblePerson=" + $rootScope.model.responsiblePerson;

		if (params == "") {
			params += "&moduleId=" + $stateParams.moduleId;
		}
		params = "iUrl=egmas/interface/list.do|iLoading=FLOAT|iParams="
				+ params;
		$rootScope.getBaseData($scope, $http, params, page);
	};
	$scope.getData();
...
mainModule
		.controller(
				'BackInterfaceEgmasDetailCtrl',
				function($rootScope, $scope, $http, $state, $stateParams,
						$http, httpService) {
					$scope.getRequestExam = function(editerId, targetId, item,
							tableId) {
						var params = "iUrl=egmas/interface/getRequestExam.do|iLoading=FLOAT|iPost=true|iParams=&"
								+ $.param($rootScope.model);
						httpService
								.callHttpMethod($http, params)
								.success(
					...
					$scope.editerParam = function(editerId, targetId, item,
						...
						// i++;
						// addOneParam(value.name,value.necessary,value.type,
						// value.def,value.remark,i,tableId)
						// });
						// }
						$("#" + editerId).removeClass('none');
						$("#" + targetId).addClass('none');
...
/** **** EGMAS 数据源控制器 *** */
mainModule.controller('egmasSourceCtrl', function($rootScope, $scope, $http,
		$state, $stateParams, $http, httpService) {
	$scope.getData = function(page) {
...
mainModule
		.controller(
				...
				function($rootScope, $scope, $http, $state, $stateParams,
						$http, httpService) {
					$scope.getData = function(page) {
						var params = "iUrl=back/appPage/list.do|iLoading=FLOAT|iParams=&module="
								+ $stateParams.search_module
								+ "&person="
								+ $stateParams.search_person;
						$rootScope.getBaseData($scope, $http, params, page);
					};
					$scope.getData();
				});
...
/** **** APP 控制器 *** */
mainModule.controller('appCtrl', function($rootScope, $scope, $http, $state,
		$stateParams, $http, httpService) {
	$scope.getData = function(page) {
		var params = "iUrl=app/list.do|iLoading=FLOAT|iParams=&module="
				+ $stateParams.search_module + "&person="
				+ $stateParams.search_person;
		$rootScope.getBaseData($scope, $http, params, page);
	};
	$scope.getData();
...
/** **** AppVersion 控制器 *** */
mainModule.controller('appVersionCtrl', function($rootScope, $scope, $http,
		$state, $stateParams, $http, httpService) {
	$scope.getData = function(page) {
		var params = "iUrl=appVersion/list.do|iLoading=FLOAT|iParams=&appID="
				+ $stateParams.appID;
		$rootScope.getBaseData($scope, $http, params, page);
	};
	$scope.getData();
