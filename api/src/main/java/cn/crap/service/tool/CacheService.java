...
public class CacheService implements ICacheService {
	...
	@Override
	public String getCxModuleName(String moduleId) {
		String name = getCxModule(moduleId).getName();
		if (MyString.isEmpty(name))
			name = "无";
		return name;
	}
	...
	@Override
	public CxModule getCxModule(String moduleId) {
		if (MyString.isEmpty(moduleId)) {
			return new CxModule();
		}

		Object obj = getDao().getObj(Const.CACHE_CX_MODULE + moduleId);
		if (obj == null) {
			CxModule module = dataCxCenterDao.get(moduleId);
			if (module == null)
				module = new CxModule();
			getDao().setObj(Const.CACHE_CX_MODULE + moduleId, module, config.getCacheTime());
			return module;
